...
class CommentAnalysisServiceTest {
    ...
    private ProcessedCommentStore store;
    ...
    void setUp() {
        ...
        store = mock(ProcessedCommentStore.class);
    ...
    void 미처리_코멘트는_분석후_Slack전송하고_처리기록한다() {
        ...
        when(groqClient.analyze(any(CommentContext.class))).thenReturn(result);

        service.analyzeAsync(EVENT);

        verify(groqClient).analyze(any(CommentContext.class));
    ...
    void 분석_실패시_Slack에_실패를_알리고_처리기록은_하지_않는다() {
        ...
        when(groqClient.analyze(any(CommentContext.class))).thenThrow(boom);
